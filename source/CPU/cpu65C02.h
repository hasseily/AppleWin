...
// TODO-MP Optimization Note: ?? Move CYC(#) to array ??
		...
		if (g_debugLogInstructions > 0)
		{
			...
			else
			{
				...
				sprintf_s(_buf, 500, "%04X: %s %02X%02X - A:%02X X:%02X Y:%02X - SP:%03X - h%02X (%03d) - ADDR:%04X - h%02X (%03d) PRE: h%04X (%05d) POST: h%04X (%05d) -> h%02X (%03d)\n",
					_origPC, _opStr.c_str(),
					*(mem + _origPC + 1), *(mem + _origPC + 2),
					_origA, _origX, _origY, _origSP, valsp, valsp, addr, val2, val2, val4pre, val4pre, val4post, val4post, valval4post, valval4post);
			}
